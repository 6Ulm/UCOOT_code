...
class BatchFUGW:
    ...
    def sinkhorn(self, dict_plan, rho1, rho2, eps, log_p, data_const, alpha):
        ...
        def get_lse(dual, pos, idx, bs):
            with torch.no_grad():
                cost = self.get_tensor_cost(local_ugw_cost, data_const, alpha, pos, idx, bs)
                if pos == 2:
                    lse = torch.logsumexp((dual + log_a)[:, None] - cost / eps, dim=0)
                elif pos == 1:
                    lse = torch.logsumexp((dual + log_b)[None, :] - cost / eps, dim=1)
    ...
    def _get_barycentre(self, data_target, dict_plan, log_p, data_const, eps, alpha):
        """
        ...
        device, dtype = u.device, u.dtype
        barycentre = torch.zeros((u.shape[0], data_target.shape[1])).to(
            device).to(dtype)  # shape n1 x d2
    ...
    def solver(
        ...
        self.get_barycentre = partial(self._get_barycentre,
                                      dict_plan=dict_pi,
                                      log_p=log_p,
                                      data_const=data_const,
                                      eps=eps,
