...
class Barycenter(MegaWass):
        ...
        for pi, w, C in zip(list_pi, list_weight, list_C):
            ...
            if force_psd:
                if isinstance(C, tuple):
                    C1, C2 = C
                    term = pi_samp.T @ (C1 @ (C2.T @ pi_samp)) / (pi1_samp[:,None] * pi1_samp[None,:]) + \
                            pi_feat.T @ (C1 @ (C2.T @ pi_feat)) / (pi1_feat[:,None] * pi1_feat[None,:])
                elif torch.is_tensor(C):
                    term = pi_samp.T @ C @ pi_samp / (pi1_samp[:,None] * pi1_samp[None,:]) + \
                            pi_feat.T @ C @ pi_feat / (pi1_feat[:,None] * pi1_feat[None,:])
                term = term / 2

            else:
                if isinstance(C, tuple):
                    C1, C2 = C
                    term = pi_samp.T @ (C1 @ (C2.T @ pi_feat)) / (pi1_samp[:,None] * pi1_feat[None,:]) # shape (n, n)
                elif torch.is_tensor(C):
                    term = pi_samp.T @ C @ pi_feat / (pi1_samp[:,None] * pi1_feat[None,:]) # shape (n, n)
    ...
    def update_bary_attr(list_pi, list_weight, list_attr):
        ...
        bary_attr = 0
        for (pi_samp, pi_feat), w, attr in zip(list_pi, list_weight, list_attr):
            pi_sum = pi_samp + pi_feat
            if attr is not None:
                bary_attr = bary_attr + w * pi_sum.T @ attr / pi_sum.sum(0).reshape(-1, 1)
    ...
    def train_individual(self, list_C, list_attr, list_p, list_params, \
        ...
        for C, C_attr, px, params, pi, dual in zip(list_C, list_attr, list_p, list_params, list_pi, list_dual):
            ...
            D = torch.cdist(C_attr, bary_attr)**2 if C_attr is not None else None
    ...
    def barycenter(self, list_weight, list_C, list_rho, list_eps, list_p=None, \
                ...
                uot_mode="entropic", entropic_mode="joint", mass_rescaling=True, force_psd=False):
        """
        (Cs, C)
        """
